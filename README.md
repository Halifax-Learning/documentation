...
-   [Deployment](#deployment)
    -   [SMU Server File Structure](#smu-server-file-structure)
...
All audio files are stored on the file system on the SMU server (the home directory is `/home/nicolas/audio`), with only file paths saved in the database tables. These files are retrieved via the GET /api/audio/{test_id} API, which provides audio files for a test. Several key optimizations ensure efficient and secure audio handling:
...
### SMU Server File Structure

The application is deployed on SMU server using Docker.

The home directory of the application on the SMU server is `/home/di/phonetic-test`. This directory contains the following subdirectories and files (only the files relevant to the deployment are listed):
...
docker-compose # the docker-compose portable executable
```

Explaination of the directories and files:

-   The `letencrypt` directory contains the SSL certificates generated by Certbot for HTTPS.
-   The `backend` directory contains the Django application.
    -   The `docker-compose.yml` file is also put inside the `deployment` folder in the backend repository.
    -   The `id_rsa` file is used for SSH connection from a docker container to the database on the SMU server. It is only needed for the manual deployment.
-   The `frontend` directory contains the `dist` folder, which contains the built React application.
    -   The `env.production` file is not needed on the server for the deployment, but it is included on the server for convenience.
-   The `docker-compose` file is the portable executable for running the Docker containers as we do not have the privilege to install docker-compose on the server.

### Docker Containers

The docker containers are organized as follows:
...
To manually deploy the application on the SMU server, follow these steps:

1. Copy the backend repository, the `.env.production` and `id_rsa` file to the `/home/di/phonetic-test/backend` directory on the server.
2. Build the frontend application on the local machine with command `npm run build`. It will require the `.env.production` file to be present in the local frontend directory in order to configure the environment variables correctly.
3. Copy the `dist` folder and the files `Dockerfile` and `nginx.conf` from the frontend directory to the `/home/di/phonetic-test/frontend` directory on the server.
4. Run docker-compose command from the `/home/di/phonetic-test` directory:

```bash
./docker-compose -f phonetic-test-backend/deployment/docker-compose.yml up --build -d
```

### Automated Deployment

We have implemented GitHub Actions pipelines to automate deployment. Whenever a merge is made into the `dev` branch of either the backend or frontend repositories, the updated code is copied to the SMU server, and the corresponding Docker container is rebuilt and updated.

Several environment variables are managed through GitHub Secrets for automated deployment. However, we still need `.env.production` file for the **backend**.

-   **Backend**: the `env.production` file is still required to be present on the server. It should be copied to the `/home/di/phonetic-test/backend` directory manually if not already present.

    _Explanation_: Technically, we can put all the environment variables in the GitHub Secrets but at this moment in the development process, for convenience, we still use the `.env.production` file for environment variables that are likely to change. Also note that `.env.production` file is not committed to the repository, that's why it is neccessary to copy it manually to the server.

-   **Frontend**: the `.env.production` file is not needed either on the server or on the local machine for automated deployment.
