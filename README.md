...
-   The `backend` directory contains the Django application. The `docker-compose.yml` file is also put inside the `deployment` folder in the backend repository.
-   The `frontend` directory contains the `dist` folder, which contains the built React application. The `env.production` file is not needed on the server for the deployment, but it is included on the server for convenience.
...
1. Copy the backend repository and the `.env.production` file to the `/home/di/phonetic-test/backend` directory on the server.
